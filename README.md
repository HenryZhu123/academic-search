...
```bash
psql -U squai_user -d squai_db -f sql/create_squai_table.sql
```
...
```bash
export SQUAI_DB_NAME=squai_db
export SQUAI_DB_USER=squai_user
export SQUAI_DB_PASSWORD=squai123
export SQUAI_DB_HOST=127.0.0.1
export SQUAI_DB_PORT=5432
```
...
```python
payload_bytes = full_text.encode("utf-8")
```

### 5) 与检索流程集成

在原有检索逻辑产出 `papers`（每项包含 `title/abstract/body_text`）后，新增一步：

```python
from scripts.fulltext_pg_store import PaperContentStore, PostgresConfig
from scripts.fulltext_fetcher import enrich_paper_with_body_text
